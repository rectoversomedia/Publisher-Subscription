...
const STATUS_CONFIG: Record<string, { bg: string; text: string; border: string; icon: React.ReactNode }> = {
  ...
  DRAFT:     { bg: 'bg-slate-50', text: 'text-slate-400', border: 'border-slate-300', icon: <FlaskConical className="w-3 h-3" /> },
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Draft', value: experiments.filter(e => e.status === 'DRAFT').length, icon: FlaskConical, color: 'text-slate-400', bg: 'bg-slate-50' },
        ].map((stat) => {
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-[13px] font-semibold text-slate-400">No experiments yet</p>
            <p className="text-[11px] text-slate-300 mt-1">Create your first A/B experiment to optimize reader revenue</p>
          </div>
        ) : (
          experiments.map((exp) => {
            ...
            return (
              <div key={exp.id} className={`bg-white border rounded-2xl overflow-hidden transition-all ${isExpanded ? 'border-slate-200' : 'border-slate-200'}`}>
                ...
                <div className="p-5" onClick={() => setExpanded(isExpanded ? null : exp.id)}>
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {exp.status === 'DRAFT' && (
                        <button onClick={(e) => { e.stopPropagation(); handleAction(exp.id, 'start'); }}
                          className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold text-slate-900 bg-red-600 rounded-lg hover:bg-[#B01028] transition-all">
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-4">
                    {variants.map((v) => (
                      <div key={v.id} className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                        <div className="text-[12px] font-semibold text-slate-600">{v.name}</div>
                        <div className="text-[10px] text-slate-300 mt-0.5">{v.allocation_percentage}% traffic</div>
                  ...
                  {/* Footer metrics */}
                  <div className="flex items-center gap-5 mt-3 text-[11px] text-slate-300 flex-wrap">
                    ...
                    <span className="text-slate-300 ml-auto cursor-pointer">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {variants.map((v) => (
                        <div key={v.id} className="bg-white border border-slate-200 rounded-xl p-4">
                          <div className="text-[12px] font-bold text-slate-700 mb-3">{v.name}</div>
                          {exp.results?.find(r => r.variant_id === v.id) ? (
                                ...
                                <div className="space-y-2">
                                  ...
                                  {[
                                    ...
                                    <div key={String(label)} className="flex items-center justify-between">
                                      ...
                                      <span className="text-[12px] font-mono font-bold text-slate-600">{String(value)}</span>
                                    </div>
                                  ))}
                                </div>
                              );
                            ...
                            <div className="text-[11px] text-slate-300 text-center py-4">No results yet</div>
                          )}
                        </div>
                      ))}
                    </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-[14px] font-bold text-slate-900">New Experiment</h2>
              </div>
              <button onClick={() => setShowCreate(false)} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-50 rounded-lg">
                <X className="w-4 h-4" />
              </button>
            </div>
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              {[
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 text-[13px] bg-slate-50 border border-slate-300 rounded-xl text-slate-900 placeholder-white/20 focus:outline-none focus:border-red-600/40 transition-all"
                  />
                </div>
              ))}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-semibold text-slate-400 mb-1.5">Primary Metric</label>
                  <select value={form.primary_metric} onChange={(e) => setForm((f) => ({ ...f, primary_metric: e.target.value }))}
                    className="w-full px-3.5 py-2.5 text-[13px] bg-slate-50 border border-slate-300 rounded-xl text-slate-900 focus:outline-none focus:border-red-600/40 transition-all appearance-none">
                    {Object.entries(METRIC_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-slate-400 mb-1.5">Traffic %</label>
                  <input type="number" min="5" max="100" value={form.traffic_percentage}
                    onChange={(e) => setForm((f) => ({ ...f, traffic_percentage: e.target.value }))}
                    className="w-full px-3.5 py-2.5 text-[13px] bg-slate-50 border border-slate-300 rounded-xl text-slate-900 focus:outline-none focus:border-red-600/40 transition-all" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {(['control_name', 'variant_name'] as const).map((key) => (
                  <div key={key}>
                    <label className="block text-[11px] font-semibold text-slate-400 mb-1.5 capitalize">{key.replace('_', ' ')}</label>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setShowCreate(false)}
                  className="px-4 py-2.5 text-[12px] text-slate-400 border border-slate-300 rounded-xl hover:bg-slate-50 transition-all">
                  Cancel
                </button>
                <button type="submit" disabled={creating || !form.name.trim()}
                  className="px-5 py-2.5 text-[12px] font-bold text-slate-900 bg-red-600 rounded-xl hover:bg-[#B01028] disabled:opacity-50 transition-all">
