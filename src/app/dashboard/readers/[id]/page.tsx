    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[13px] font-mono font-bold text-slate-800">{value}</span>
          {sublabel && <span className="text-[10px] text-slate-300">{sublabel}</span>}
        </div>
      </div>
...
export default function ReaderDetailPage() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-64">
      <div className="flex flex-col items-center gap-3">
        ...
        <span className="text-[12px] text-slate-400">Loading reader profile…</span>
      </div>
    </div>
  );
  ...
  if (!data?.reader) return (
    <div className="text-center py-20">
      ...
      <p className="text-[14px] font-semibold text-slate-400">Reader not found</p>
      <Link href="/dashboard/readers" className="text-[12px] text-red-400 hover:text-red-600 mt-2 inline-block">Back to readers</Link>
    </div>
  );
  ...
  const getDecisionColor = (d: string) => {
    if (d === 'SHOW_MONTHLY' || d === 'SHOW_ANNUAL') return 'text-red-500 bg-red-50 border border-red-200';
    if (d === 'SHOW_REGISTRATION') return 'text-blue-400 bg-blue-500/15 border border-blue-500/20';
    return 'text-slate-400 bg-slate-50 border border-slate-300';
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-4">
        <Link href="/dashboard/readers" className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-50 border border-slate-300 text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-all">
          <ArrowLeft className="w-4 h-4" />
        </Link>
        <div className="flex-1 min-w-0">
          <h1 className="text-[20px] font-black text-slate-900 tracking-tight truncate">{displayName}</h1>
          <p className="text-[11px] text-slate-300 font-mono mt-0.5">
            {reader.anonymous_id ? `anon_${reader.anonymous_id.slice(-8)}` : id?.slice(0, 16) + '…'}
            <button onClick={fetchData} className="ml-3 text-slate-300 hover:text-slate-400 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-red-500/30 to-red-500/10 border border-red-200 flex items-center justify-center">
                <User className="w-5 h-5 text-red-500" />
              </div>
              <div>
                <div className="text-[13px] font-bold text-slate-900">{displayName}</div>
                <div className={`text-[10px] font-semibold mt-0.5 ${isKnown ? 'text-blue-400' : 'text-slate-300'}`}>
            ...
            {isKnown && (reader.email || reader.phone || reader.name) && (
              <div className="p-3 bg-red-600/8 border border-red-100 rounded-xl mb-4">
                <div className="text-[9px] font-bold uppercase tracking-widest text-red-400 mb-2 px-1">Contact Info</div>
                {reader.name && (
                  <div className="flex items-center gap-2.5 py-1.5 border-b border-slate-200 last:border-0">
                    <User className="w-3.5 h-3.5 text-blue-400 flex-shrink-0" />
                    <div>
                      <div className="text-[9px] text-slate-300">Name</div>
                      <div className="text-[12px] font-semibold text-slate-800">{reader.name}</div>
                    </div>
                  </div>
                )}
                {reader.email && (
                  <div className="flex items-center gap-2.5 py-1.5 border-b border-slate-200 last:border-0">
                    ...
                    <div>
                      <div className="text-[9px] text-slate-300">Email</div>
                      <div className="text-[12px] font-semibold text-slate-800">{reader.email}</div>
                    </div>
                  </div>
                )}
                {reader.phone && (
                  <div className="flex items-center gap-2.5 py-1.5">
                    ...
                    <div>
                      <div className="text-[9px] text-slate-300">Phone</div>
                      <div className="text-[12px] font-semibold text-slate-800">{reader.phone}</div>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-2.5">
              {[
                ['Identity Status', reader.identity_status, isKnown ? 'text-blue-400' : 'text-slate-400'],
                ['Subscription', reader.subscription_status,
                  reader.subscription_status === 'ACTIVE' ? 'text-emerald-400' :
                  reader.subscription_status === 'EXPIRED' ? 'text-red-400' : 'text-slate-400'],
                ['Last Active', timeAgo(reader.last_seen_at), 'text-slate-400'],
                ['Member Since', timeAgo(reader.created_at), 'text-slate-400'],
              ].map(([label, value, colorClass]) => (
                <div key={String(label)} className="flex items-center justify-between">
                  <span className="text-[11px] text-slate-400">{String(label)}</span>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-[12px] font-bold text-slate-700">Revenue Brain Scores</h2>
            </div>
          ...
          {/* Estimated LTV */}
          <div className="bg-gradient-to-br from-red-500/10 to-red-500/5 border border-red-200 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-2">
              <DollarSign className="w-4 h-4 text-emerald-400" />
              <span className="text-[11px] text-slate-400">Estimated Lifetime Value</span>
            </div>
            <div className="text-[28px] font-black text-slate-900 font-mono tracking-tight leading-none mb-1">
              {features?.predicted_ltv ? fmtRp(features.predicted_ltv) : '—'}
            </div>
            <div className="text-[10px] text-slate-300">Heuristic estimate · not guaranteed</div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-[12px] font-bold text-slate-700">Behavioral Metrics (30d)</h2>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                [features?.checkout_starts_30d ?? 0, 'Checkout Starts', <ShoppingCart key="cs" className="w-3.5 h-3.5 text-red-600" />],
                [features?.days_since_last_visit ?? 0, 'Days Since Visit', <Clock key="ds" className="w-3.5 h-3.5 text-slate-400" />],
              ].map(([value, label, icon]) => (
                <div key={String(label)} className="flex items-center gap-2.5 p-2.5 bg-slate-50 rounded-xl border border-slate-200">
                  <div className="flex-shrink-0">{icon}</div>
                  <div>
                    <div className="text-[13px] font-mono font-bold text-slate-800">{String(value)}</div>
                    <div className="text-[9px] text-slate-300">{String(label)}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-[12px] font-bold text-slate-700">Content Topic Affinity</h2>
            </div>
            <div className="space-y-3">
              {topics.slice(0, 8).map((topic) => (
                <div key={topic.topic}>
                  <div className="flex justify-between text-[11px] mb-1">
                    ...
                    <span className="text-slate-400 font-mono">{Math.round(topic.score)}</span>
                  </div>
                  <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div
              ...
              {topics.length === 0 && (
                <div className="text-[12px] text-slate-300 text-center py-6">No topic data yet</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {data.latest_decision_explanation && data.decisions?.[0] && (
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-[12px] font-bold text-slate-700">Recommended Action</h2>
              </div>
          ...
          {data.decisions?.[0]?.reason_codes && data.decisions[0].reason_codes.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              <h2 className="text-[12px] font-bold text-slate-700 mb-3">Decision Factors</h2>
              <div className="flex flex-wrap gap-1.5">
                {data.decisions[0].reason_codes.map((code) => (
                  <span key={code} className="text-[10px] px-2.5 py-1 bg-slate-50 text-slate-400 border border-slate-300 rounded-lg font-mono">
          ...
          {data.decisions?.[0]?.score_snapshot && Object.keys(data.decisions[0].score_snapshot).length > 0 && (
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              <h2 className="text-[12px] font-bold text-slate-700 mb-3">Score Snapshot at Decision</h2>
          ...
          {/* Event Timeline */}
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <h2 className="text-[12px] font-bold text-slate-700 mb-4 flex items-center gap-2">
              <Clock className="w-3.5 h-3.5 text-blue-400" />
              Recent Activity
              <span className="ml-auto text-[10px] text-slate-300">{data.events.length} events</span>
            </h2>
              ...
              {data.events.slice(0, 20).map((event) => (
                <div key={event.event_id} className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-slate-300 font-mono">{timeAgo(event.timestamp)}</div>
                  </div>
                </div>
              ))}
              {data.events.length === 0 && (
                <div className="text-[12px] text-slate-300 text-center py-6">No events recorded</div>
              )}
            </div>
          </div>
        </div>
